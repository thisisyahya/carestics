...
const allProducts = [
  ...
  { 
    ...
    category: '3D Posters', 
    price: '$120', 
    description: 'A unique piece of three-dimensional wall art featuring a highly detailed 1:32 scale diecast sports car physically mounted onto a premium printed graphic backdrop.', 
  ...
  { 
    ...
    category: '3D Posters', 
    price: '$140', 
    description: 'Blurring the line between scale models and wall decor. This display features a real 1:34 diecast car permanently secured to a sleek, automotive-themed poster for a literal pop-out effect.', 
    image: '/hero/hero-p4.png' 
  }
];

const categories = ['All', 'Car Chamber', 'Posters', '3D Posters'];
...
export default function ProductsPage() {
  ...
  return (
    <main className="relative min-h-screen bg-studio-950 text-studio-100 selection:bg-amber-500/20 selection:text-amber-400 overflow-hidden">
      ...
      {/* Navbar */}
      <nav className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-4 sm:px-8 py-4 sm:py-6 backdrop-blur-xl bg-studio-950/50 border-b border-studio-800/50">
        <Link href="/" className="text-xl sm:text-2xl text-white tracking-tighter">
      ...
      <section className="relative z-10 pt-24 pb-12 px-3 sm:pt-40 sm:pb-20 sm:px-6 max-w-7xl mx-auto">
        ...
        {/* Compressed Mobile Grid (3 Columns on sm+, 2 Columns on Mobile for maximum card visibility) */}
              ...
              >
                <div className="relative w-full h-28 sm:h-52 md:h-[22rem] overflow-hidden bg-studio-950 shrink-0">
                  ...
                  <div className="absolute top-2 right-2 sm:top-4 sm:right-4 bg-studio-950/80 backdrop-blur-md border border-studio-800 px-2 py-0.5 sm:px-4 sm:py-1.5 rounded-full">
                    <span className="text-[10px] sm:text-sm font-medium text-studio-100">{product.price}</span>
                  </div>
                </div>
